...
def numpy_to_int(dict_np):
    for key,item in list(dict_np.items()):
...
def ajax_source_month_stack(request):
    ...
    team = df_month['受检单位'][df_month['信息来源']=="班组自查"].value_counts().to_dict()
    workshop = df_month['受检单位'][df_month['信息来源']=="车间监管"].value_counts().to_dict()
    quality = df_month['受检单位'][df_month['信息来源']=="质量监管"].value_counts().to_dict()
...
def ajax_team_month_stack(request):
    ...
    team = df_month['责任班组'][df_month['信息来源']=="班组自查"].value_counts().to_dict()
    workshop = df_month['责任班组'][df_month['信息来源']=="车间监管"].value_counts().to_dict()
    quality = df_month['责任班组'][df_month['信息来源'] == "质量监管"].value_counts().to_dict()
    team = numpy_to_int(team)
    workshop = numpy_to_int(workshop)
    quality = numpy_to_int(quality)
...
def self_inspect_trendence(request, workshop_name):
    if workshop_name == "1":
        list_team_name = ['航线一（1）', '航线一（2）', '航线一（3）', '航线一（4）']
        team_name = '航线一'
    elif workshop_name == "2":
        list_team_name = ['航线二（1）', '航线二（2）', '航线二（3）', '航线二（4）']
        team_name = '航线二'
    else:
        list_team_name = ['航线三（1）', '航线三（2）', '航线三（3）', '航线三（4）']
        team_name = '航线三'
    df_data = pd.read_hdf('data.h5', 'df')
    df_da = pd.DataFrame(list_all_data(), index=df_data['日期'])
    df_string = df_da[df_da['受检单位']==team_name]
    string_index = df_string['日期']
    # 计算出起止月份
    ...
    for r in arrow.Arrow.range('month', start_month, end_month):
        ...
        try:
            ...
            for item in list_team_name:
                df_month_airline_1 = df_month[df_month['责任班组']==item]
                total_count = df_month_airline_1.shape[0]
                dict_data[item]['total'].append(total_count)
                df_month_self_inspect_count = df_month_airline_1[df_month_airline_1['信息来源']=='班组自查']
...
def grade_scatter(request):
    df_data = pd.read_hdf('data.h5', 'df')
    df_data = df_data[df_data["严重程度"]>0]
    df_data = df_data[["日期","受检单位","严重程度"]]
    df_airline_1 = df_data[df_data["受检单位"]=="航线一"]
    json_airline_1 = df_airline_1[["日期","严重程度"]].to_json(orient="values")
    df_airline_2 = df_data[df_data["受检单位"]=="航线二"]
    json_airline_2 = df_airline_2[["日期","严重程度"]].to_json(orient="values")
    df_certain = df_data[df_data["受检单位"]=="定检"]
    json_certain = df_certain[["日期","严重程度"]].to_json(orient="values")
    df_other = df_data[(df_data["受检单位"]!="航线一")&(df_data["受检单位"]!="航线二")&(df_data["受检单位"]!="定检")]
    json_other = df_other[["日期","严重程度"]].to_json(orient="values")
